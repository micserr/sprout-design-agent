...
## [2.0.2](https://github.com/micserr/sprout-design-agent/compare/v2.0.1...v2.0.2) (2026-06-17)


### Bug Fixes

* sync plugin.json version to 2.0.1 ([#35](https://github.com/micserr/sprout-design-agent/issues/35)) ([d3adb46](https://github.com/micserr/sprout-design-agent/commit/d3adb46945deade8e108c4f849185ebf53e3df05))
...
### Changed
- **`toge:prototype` install discipline hardened.** Confirmed per-component install (only what the flow needs) as the correct default and made the discipline continuous: a new "Install discipline" block in Step 2 adds an upfront component manifest, a *per-component install-and-read gate that fires before every `<Toge*>` tag on any screen* (not just at Step 0 pre-flight), a "no read, no tag" hard rule, and a "verify against the installed directory, not the README" check. Reconciled the contradictory Step 0 check 6 (dropped "run the bulk installer first" → install missing components per-component) and clarified that the registry is remote and pulls into `src/components/ui/` rather than into `toge-ds-components`.
- **Enriched the Toge component catalog** in `guide/toge-design-system/README.md` — the bare name→slug table now carries a one-line *what it is · when to pick it over its closest sibling* for every primitive (sourced from the Toge registry's disambiguating descriptions), and adds the missing `platform/global-search` block. Gives the `toge:prototype` skill real selection guidance to reduce wrong-component picks, without touching the MCP guard rule.
